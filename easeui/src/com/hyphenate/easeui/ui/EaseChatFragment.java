...
public class EaseChatFragment extends EaseBaseFragment {
    ...
    protected void initView() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ((InputMethodManager)getActivity().getSystemService(getActivity().INPUT_METHOD_SERVICE)).showSoftInput(inputMenu, 0);
        }
    ...
    protected void setUpView() {
        ...
        titleBar.setLeftImageResource(R.drawable.ic_topbar_back);
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            // 设置标题
//            if(EaseUserUtils.getUserInfo(toChatUsername) != null){
//                titleBar.setTitle(EaseUserUtils.getUserInfo(toChatUsername).getNick());
//            }
            HuanxinUserInfoCacheEntity huanxinUserInfoCacheEntity = HuanxinUserInfoCacheUtil.getHuanxinUserInfoCacheEntity(getActivity());
            if(huanxinUserInfoCacheEntity!=null&&huanxinUserInfoCacheEntity.map!=null&&huanxinUserInfoCacheEntity.map.size()>0){
                EaseUser easeUser = huanxinUserInfoCacheEntity.map.get(toChatUsername);
                if(null!=easeUser){
                    titleBar.setTitle( easeUser.getNick());
                }
            }
            ...
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                // 群聊
//                EMGroup group = EMClient.getInstance().groupManager().getAllGroups().get(0);
//                if (group != null)
//                    titleBar.setTitle(group.getGroupName());
    ...
    protected void sendMessage(EMMessage message){
        sign="haveSend"; //页面销毁的时候就不用再发“大家好”了
        if(chatFragmentListener != null){
            //设置扩展属性
